...
class AddMetaFormMobile extends React.Component<any, any> {
    ...
    onSubmit = values => {
        const {transaction, selectedOutputIndex} = this.props.history.location.state;
        ...
        this.supplyChainService.appendMetaData(transaction.transactionId, selectedOutputIndex, requestModel).subscribe(res => { // selectedOutputIndex is different, transactionId are the same
            if (res === 'error') {
                ...
                this.setState({
                    ...
                    returnedTransaction: res.model.transactionId
                });
            }
        });
    }
    ...
    render() {
        const {transaction, divideTransactions, selectedOutputIndex} = this.props.history.location.state;
        const resource = ResourceManager.getResource();
        return (
            <>
                <CssBaseline/>
                <header style={{backgroundColor: '#2979ff'}}>
                    <Tooltip title='More Asset Information'>
                        <Typography style={{textAlign: 'center', color: '#fff'}} variant='h2' component='h2'>{resource.add_meta_form}
                            <br/>
                        </Typography>
                    </Tooltip>
                </header>
                            ...
                            <form onSubmit={handleSubmit} noValidate autoComplete={'off'}>
                                <Paper style={{padding: 16, boxShadow: 'none'}}>
                                    <Grid container alignItems='flex-start' spacing={4}>
                                                ...
                                                <Grid item xs={12}>
                                                    <Card className='custom-add-product-card'
                                                        ...
                                                        <CardContent>
                                                            <Grid item xs={12}>
                                                                <FormControl>
                                                                    <InputLabel shrink={true} id='product-select'>Chọn
                                                                        sản phẩm</InputLabel>
                                                                    <MatSelect
                                                                        id='product-select'
                                                                        displayEmpty
                                                            ...
                                                            <Grid item xs={12}>
                                                                <InputLabel shrink={true} id='item-id-select'>Chọn mã
                                                                    sản phẩm</InputLabel>
                                                                <FormControl>
                                                                    <MatSelect
                                                                        id='item-id-select'
                                                                    ...
                                                                    >
                                                                        <MenuItem value=''>&nbsp;</MenuItem>
                                                                        {this.state.productItems.map((item, i) =>
                                                                            <MenuItem
                                                                                value={item._id}
                                        ...
                                        >
                                            <DialogTitle style={{textAlign: 'center'}} id='alert-dialog-title'><InfoIcon
                                                style={{fontSize: 64}}
                                        ...
                                        >
                                            <DialogTitle style={{textAlign: 'center'}}
                                                ...
                                                <ClearOutlineIcon style={{fontSize: 64}}
                                                                  className='dialog-icon-fail'/>}</DialogTitle>
                                            <DialogContent>
    ...
    private handleCloseResultDialog(isAddAssetSuccessful: any) {
        this.setState({resultDialogOpen: false}, () => {
            if (isAddAssetSuccessful) {
                this.props.history.push(`/getHisAssetByTransId?currentTransId=${this.state.returnedTransaction}`);
            }
        });
    }
}

export default withRouter(AddMetaFormMobile);
